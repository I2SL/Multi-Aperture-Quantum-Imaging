...
src_coords_frac = genMinDistConstellation(src_brites,1/32,1); % source coordinates in fractional rayleigh units [x/rl]. The rayleigh length corresponds to the effective diameter
...
n_pho = 2e5;        % mean photon number                   [integer]
...
pos_samp = 101;     % image-plane samples along each axis  --> Image plane for source position estimates has dimensions [img_samp,img_samp]

...
src_coords = genMinDistConstellation(src_brites,.3,1); % source coordinates in fractional rayleigh units [x/rl]. The rayleigh length corresponds to the effective diameter
...
n_pho = 5e5;        % mean photon number                   [integer]
...
subap_samp = 67;   % sub-aperure samples along each axis  --> Bounding box for each aperture has dimensions [subap_samp,subap_samp]
